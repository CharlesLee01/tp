...
<div markdown="span" class="alert alert-info">:information_source: **Note:** An alternative (arguably, a more OOP) model is given below. It has a `Tag` list in the `Pawbook`, which `Entity` references. This allows `Pawbook` to only require one `Tag` object per unique `Tag`, instead of each `Entity` needing their own `Tag` object.<br>
...
* can save the pawbook data in json format and read it back.
...
The proposed undo/redo mechanism is facilitated by `VersionedPawbook`. It extends `Pawbook` with an undo/redo history, stored internally as an `PawbookStateList` and `currentStatePointer`. Additionally, it implements the following operations:

* `VersionedPawbook#commit()` — Saves the current pawbook state in its history.
* `VersionedPawbook#undo()` — Restores the previous pawbook state from its history.
* `VersionedPawbook#redo()` — Restores a previously undone pawbook state from its history.

These operations are exposed in the `Model` interface as `Model#commitPawbook()`, `Model#undoPawbook()` and `Model#redoPawbook()` respectively.

Given below is an example usage scenario and how the undo/redo mechanism behaves at each step.

Step 1. The user launches the application for the first time. The `VersionedPawbook` will be initialized with the initial pawbook state, and the `currentStatePointer` pointing to that single pawbook state.

![UndoRedoState0](images/UndoRedoState0.png)

Step 2. The user executes `delete owner 1` command to delete the 5th owner in the pawbook. The `delete` command calls `Model#commitPawbook()`, causing the modified state of the pawbook after the `delete 5` command executes to be saved in the `pawbookStateList`, and the `currentStatePointer` is shifted to the newly inserted pawbook state.

![UndoRedoState1](images/UndoRedoState1.png)

Step 3. The user executes `add owner n/John p/1234 e/john@email.com Clementi Ave 2 t/friends` to add a new owner. The `add` command also calls `Model#commitPawbook()`, causing another modified pawbook state to be saved into the `pawbookStateList`.

![UndoRedoState2](images/UndoRedoState2.png)

<div markdown="span" class="alert alert-info">:information_source: **Note:** If a command fails its execution, it will not call `Model#commitPawbook()`, so the pawbook state will not be saved into the `pawbookStateList`.

</div>

Step 4. The user now decides that adding the owner was a mistake, and decides to undo that action by executing the `undo` command. The `undo` command will call `Model#undoPawbook()`, which will shift the `currentStatePointer` once to the left, pointing it to the previous pawbook state, and restores the pawbook to that state.

![UndoRedoState3](images/UndoRedoState3.png)

<div markdown="span" class="alert alert-info">:information_source: **Note:** If the `currentStatePointer` is at index 0, pointing to the initial Pawbook state, then there are no previous Pawbook states to restore. The `undo` command uses `Model#canUndoPawbook()` to check if this is the case. If so, it will return an error to the user rather
...
The `redo` command does the opposite — it calls `Model#redPawbook()`, which shifts the `currentStatePointer` once to the right, pointing to the previously undone state, and restores the pawbook to that state.

<div markdown="span" class="alert alert-info">:information_source: **Note:** If the `currentStatePointer` is at index `pawbookStateList.size() - 1`, pointing to the latest pawbook state, then there are no undone Pawb
ook states to restore. The `redo` command uses `Model#canRedoPawbook()` to check if this is the case. If so, it will return an error to the user rather than attempting to perform the redo.

</div>

Step 5. The user then decides to execute the command `list`. Commands that do not modify the pawbook, such as `list`, will usually not call `Model#commitPawbook()`, `Model#undoPawbook()` or `Model#redoPawbook()`. Thus, the `pawbookStateList` remains unchanged.

![UndoRedoState4](images/UndoRedoState4.png)

Step 6. The user executes `clear`, which calls `Model#commitPawbook()`. Since the `currentStatePointer` is not pointing at the end of the `pawbookStateList`, all pawbook states after the `currentStatePointer` will be purged. Reason: It no longer makes sense to redo the `add n/David …​` command. This is the behavior that most modern desktop applications follow.
...
* **Alternative 1 (current choice):** Saves the entire pawbook.
